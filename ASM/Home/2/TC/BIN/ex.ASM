...
;Function to remove the 13,10 from the string,the ENTER
	...
	RET
	DELETE_ENTER ENDP
;Function to check that the string is made of digits and not other chars
	DigitCheck PROC NEAR
	MOV BL,1				;setting flag to 1 
	LEA SI,source
	MOV CX,AX
	isDigit:
		CMP [SI],BYTE PTR '0'
	...
	Terminate:
		...
		MOV DX,OFFSET InvaliDigit		; Point display string to InvaliDigit
		INT 21H			; Print  chosen message
		MOV BL,0			;flag = 0 - other unwanted chars were found
		JMP TryAgain		
	CONTINUE2:
		INC SI
		LOOP isDigit		;the loop goes through all the chars in the source string
	TryAgain:
	RET
	DigitCheck ENDP
...
; Calling to a function to remove the 13,10 from the string
	...
	CALL DigitCheck	
	CMP BL,BYTE PTR 0		;checking the flag from DigitCheck function 0-other chars no from 0-9 were found, 1-The string is Ok
	JE inputNum
	LEA DI,PrintNum
	LEA SI,source
	MOV CX,AX				;mov the string byte length to cx for the rep loop
;copy the source string to printNum string
	REP MOVSB
;check if the sum of the digits is odd or not without changing the AL register
	TEST AL,1
;Moving the num one byte forward and add 0 at the beginning 
	JZ EVEN1
		MOV CX,AX
		LEA SI,source
		ADD SI,AX
